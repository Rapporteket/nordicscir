...
EkstrVar <- c('OpphTot', 'DagerRehab','DagerTilRehab','Permisjon')
grenser <- c(365,365,80,20)
names(grenser) <- EkstrVar

#NB: Må ta høyde for at ikke ekstremobservasjoner !!!!!!!!!!!!!!
Avv <- t((t(NSdataEget[ ,EkstrVar]) <= c(0,-1,-1,-1) ) | (t(NSdataEget[ ,EkstrVar]) > grenser))
...
EkstrTab <- rbind(if (AntAvv[1]>0) {cbind(NSdataEget[Avv[,1]==TRUE, 'SkjemaGUID'],
			...
			'Dager til rehab', as.numeric(NSdataEget[Avv[,3]==TRUE, EkstrVar[3]]))},
				if (AntAvv[4]>0) {cbind(NSdataEget[Avv[,4]==TRUE, c('SkjemaGUID')],
...
<<FigPermisjon, echo=FALSE>>=
	NSFigAndeler(NSdata,   valgtVar='Permisjon', datoFra=datoFra, datoTil=datoTil,
			traume=traume, enhetsUtvalg=1, reshID=reshID, preprosess=0,hentData=0)
@
}
...
<<FigAlderGjsn, echo=FALSE>>=
	NSFigGjsnGrVar(NSdata, valgtVar='Alder', datoFra=datoFra, datoTil=datoTil,
		valgtMaal='', minald=minald, maxald=maxald, erMann=erMann, traume=traume, preprosess=0, hentData=0)
@
}
...
<<FigPermisjonGjSn, echo=FALSE>>=
	NSFigGjsnGrVar(NSdata,   valgtVar='Permisjon', datoFra=datoFra, datoTil=datoTil,
		valgtMaal='', minald=minald, maxald=maxald, erMann=erMann, traume=traume, preprosess=0, 
    hentData=0)
@
}
