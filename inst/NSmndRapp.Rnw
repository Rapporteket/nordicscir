...
fjernSide <- ifelse(nordisk==1, '<0>[noframenumbering]', '[fragile]')
Komplikasjoner <- ifelse(nordisk==1, '', 'Komplikasjoner')
utNord <- ifelse(nordisk==1, '- ikke for nordisk', '')

#Bare gyldige kontrollskjema:
if (nordisk == 0) {
KontrollH <- KontrollH[KontrollH$ControlStatus==0, ]}

#!!!!!!!!NB: Kanskje må dette gjøre i preprosesseringa!! I så fall må inn/ut inngå som inngangsparameter i funksjonen
...
HovedSkjema$Aar <- lubridate::year(HovedSkjema$DischgDt) #AdmitDt)
...
\item Livskvalitet
\item Komplikasjoner \Sexpr{utNord}
\item Anbefalt tid til kontroll \Sexpr{utNord}
\end{itemize}
...
xtable::xtable(tabTilknKtrSkjema$Antall, digits=0, align=c('l', rep('r', ncol(tabTilknKtrSkjema$Antall))),
		caption=paste0('Antall kontrollskjema og antall av disse som har tilknyttede skjema. Pasienter utskrevet  fra og med ', datoFra, '.
		               (Basert på hovedskjema) DENNE TABELLEN MÅ SJEKKES HVIS DEN SKAL VÆRE MED'))

xtable::xtable(tabTilknKtrSkjema$Andeler, digits=1, align=c('l', rep('r', ncol(tabTilknKtrSkjema$Andeler))),
		caption=paste0('Andel (prosent) av kontrollskjemaene som har ulike typer oppfølgingsskjema.DENNE TABELLEN MÅ SJEKKES HVIS DEN SKAL VÆRE MED'))
...
%  		caption='Andel pasienter (prosent) som til nå har hatt kontroll, per utskrivingsår og avdeling.')
%
% \end{frame}
...
NSFigGjsnGrVar(RegData=HovedSkjema12mnd, valgtVar='RegForsinkelse', valgtMaal='gjsn', grVar=grVar,
                              datoFra=datoFra, datoTil=datoTil-30, datoUt=1,
...
\section{Tarmfunksjon}
<<'Figurerrr', results='asis'>>=

      NSFigAndeler(RegData=TarmH, valgtVar='TarmNBD',
                   datoFra=datoFra, datoTil=datoTil, datoUt = 1,
                   outfile='TarmNBD.pdf',
                   reshID=reshID, enhetsUtvalg=enhetsUtvalg,
                   hentData=0, preprosess=0)

      NSFigAndeler(RegData=HovedSkjema12mnd, valgtVar='KomplPrim',
                   datoFra=datoFra, datoTil=datoTil, datoUt = 1,
                   outfile='KomplPrim.pdf',
                   reshID=reshID, enhetsUtvalg=enhetsUtvalg,
                   hentData=0, preprosess=0)
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.35]{TarmNBD.pdf}
\caption{Neurogenic Bowel Dysfunction (NBD) skår kategoriserer alvorlighetsgraden
(svært liten/liten/moderat/alvorlig) av nevrogen tarmdysfunksjon. }
\end{figure}
\end{frame}
...
<<'FigLivskval', results='asis'>>=

variabler <- c('LivsGen', 'LivsFys', 'LivsPsyk', 'LivsSosLiv')
for (valgtVar in variabler) {
      outfile <- paste0(valgtVar, '.pdf')
      NSFigAndeler(RegData=LivskvalH, valgtVar=valgtVar,
                   datoFra=datoFra, datoTil=datoTil, datoUt = 1,
                   outfile=outfile,
                   reshID=reshID, enhetsUtvalg=enhetsUtvalg,
                   hentData=0, preprosess=0)
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.35]{LivsGen.pdf}
\caption{Tilfredshet med livet}
\end{figure}
\end{frame}
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.35]{LivsFys.pdf}
\caption{Tilfredshet med fysisk helse}
\end{figure}
\end{frame}
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.35]{LivsPsyk.pdf}
\caption{Tilfredshet med psykisk helse}
\end{figure}
\end{frame}
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.35]{LivsSosLiv.pdf}
\caption{Tilfredshet med sosialt liv}
\end{figure}
\end{frame}


\section{\Sexpr{Komplikasjoner}}

\begin{frame} \Sexpr{fjernSide}
\begin{figure}[ht]
\centering
\includegraphics[scale=0.35]{KomplPrim.pdf}
\caption{Beskrivelse av de ulike komplikasjoner er beskrevet i registerets
         brukermanual på www.norscir.no}
\end{figure}
\end{frame}


\begin{frame} \Sexpr{fjernSide}
\section{Anbefalt tid til kontroll}
\begin{figure}[ht]
\centering
