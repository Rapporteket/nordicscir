...
fjernSide <- ifelse(nordisk==1, '<0>[noframenumbering]', '')
Komplikasjoner <- ifelse(nordisk==1, '', 'Komplikasjoner')
pktKtrKompl <- ifelse(nordisk==0,, '')
...
\item Livskvalitet
\item Komplikasjoner - (ikke for nordisk)
\item Anbefalt tid til kontroll - (ikke for nordisk)
\end{itemize}
