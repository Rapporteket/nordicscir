...
if (!exists('HovedSkjema')){
      ...
      qLivs <- paste0('SELECT  UPPER(HovedskjemaGUID) AS HovedskjemaGUID, UPPER(SkjemaGUID) 
...
Kontroll <- Kontroll[Kontroll$NoControl=='False', ]
...
\begin{frame}[fragile] {Kontrollskjema er nå filtrert på NoControll=FALSE}
Ikke vise i endelig versjon} #[exclude]
...
\begin{frame}[fragile]{Tas ut til koblingsnøkkel for kontrollskjema er riktig. Har filtrert kontrollskjema på NoControl=FALSE}
