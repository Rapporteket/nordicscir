...
HovedSkjema$MndNum <- HovedSkjema$DischgDt$mon +1
...
HovedSkjema$Aar <- 1900 + HovedSkjema$AdmitDt$year
...
Dette er en sammenstilling av resultater  fra NorSCIR. Alle registreringer er basert på ferdigstilte registreringer i registeret per rapportdato. Alle data er ikke nødvendigvis kvalitetssikret enda. Tidsutvalg for rapportene er spesifisert for hver enkelt figur.
...
RaaTab <- data.frame(Sykehus = HovedSkjema12mnd$ShNavn,
      ...
      #Kontroll = HovedSkjema12mnd$SkjemaGUID %in% KontrollH$HovedskjemaGUID,
...
variabler <- c('SkadeArsak', 'Ntsci', 'AAis', 'FAis','RegForsinkelse')
...
#variabler <- c('DagerTilRehab', 'DagerRehab')

# for (valgtVar in variabler) {
#       for (traume in c('ja', 'nei')){
#             outfile <- paste0(valgtVar, '_t_', traume,'.pdf')
#             NSFigAndeler(RegData=HovedSkjema12mnd, valgtVar=valgtVar, preprosess=0,
#                          datoFra=datoFra, datoTil=datoTil, datoUt = 1,
#                          traume = traume, outfile=outfile, reshID=reshID, enhetsUtvalg=enhetsUtvalg,
#                          )
#             }
#       }
@
...
#
#       Utvalg <- NSUtvalg(HovedSkjema, datoFra = datoFra, datoTil = datoTil, datoUt = 1)
...
NevrKlass <- lagTabNevrKlass(HovedSkjema = HovedSkjema12mnd, datoFra=datoFra, datoTil=Sys.Date(), datoUt=0)
...
xtable::xtable(NevrKlass28, digits=0, align=c('l', rep('r', ncol(NevrKlass28))),
		caption=paste0('Nevrologisk klassifikasjon for pasienter utskrevet pasienter fra og med ',
...
Liggetider <- function(RegData, datoFra='2018-01-01', datoTil=Sys.Date(), datoUt=1,
      ...
      RegData <- NSUtvalg(RegData=RegData, datoFra=datoFra, datoTil = datoTil, datoUt = 1,
