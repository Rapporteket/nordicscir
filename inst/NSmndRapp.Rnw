...
#       
#       LivskvalitetH <- rapbase::LoadRegData(registryName, qLivs, dbType)
...
RaaTab <- data.frame(Sykehus = HovedSkjema12mnd$ShNavn,
      ...
      LivskvalitetH = HovedSkjema12mnd$SkjemaGUID %in% LivskvalitetH$HovedskjemaGUID,
...
RaaTabKtr <- data.frame(Sykehus = Kontroll$HealthUnitShortName,
      #Aar = as.POSIXlt(Hskjema$AdmitDt, format="%Y-%m-%d")$year +1900,
      LivskvalitetH = Kontroll$SkjemaGUID %in% LivskvalitetH$HovedskjemaGUID,
      UrinH = Kontroll$SkjemaGUID %in% UrinH$HovedskjemaGUID,
      TarmH = Kontroll$SkjemaGUID %in% TarmH$HovedskjemaGUID,
      Funksjon = Kontroll$SkjemaGUID %in% AktivFunksjonH$HovedskjemaGUID,
...
xtable::xtable(LiggetiderMtraume, digits=0, align=c('l', rep('r', ncol(LiggetiderMtraume))),
		caption=paste0('Liggetider ved eget sykehus, pasienter med traume. Innleggelser fra og med ',datoFra,'.'))
...
xtable::xtable(LiggetiderUtraume, digits=0, align=c('l', rep('r', ncol(LiggetiderMtraume))),
		caption=paste0('Liggetider eget sykehus, pasienter uten traume. Innleggelser fra og med ',datoFra,'.'))
