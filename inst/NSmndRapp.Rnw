...
nordisk <- ifelse(sum(as.numeric(unique(HovedSkjema$LandKode)), na.rm = T)==1, 0, 1)
...
xtable::xtable(tabTilknKtrSkjema$Antall, digits=0, align=c('l', rep('r', ncol(tabTilknKtrSkjema$Antall))),
		caption=paste0('Antall kontrollskjema og antall av disse som har tilknyttede skjema.
		Tabellen oppsummerer pasienter som har vært til kontroll  fra og med ', datoFra, '.'))

xtable::xtable(tabTilknKtrSkjema$Andeler, digits=1, align=c('l', rep('r', ncol(tabTilknKtrSkjema$Andeler))),
		caption=paste0('Andel (prosent) av kontrollskjemaene som har ulike typer oppfølgingsskjema.'))
