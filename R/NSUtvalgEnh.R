...
#'                       2:eget sykehus
#'                       1-2 som registrerer i hvert av de andre land. Venter med flere sammenligninger
...
NSUtvalgEnh <- function(RegData, datoFra='2010-01-01', datoTil=Sys.Date(), datoUt=0,
	...
	indAld <- which(RegData$Alder >= minald & RegData$Alder <= maxald)
      ...
      alleTxt <- ifelse(sum(as.numeric(unique(RegData$LandKode)))==1, 'resten av landet', 'resten av norden')


      indEgen1 <- match(reshID, RegData$ReshId)
      hovedgrTxt <- ifelse(enhetsUtvalg %in% c(1,2),	#Involverer egen enhet
            as.character(RegData$ShNavn[indEgen1]), c('Hele landet', 'Hele Norden')[nordisk+1])
      ...
      if (enhetsUtvalg ==1 ) {	#Egen mot resten
				...
				medSml <- 1
				smltxt <- c('landet forøvrig', 'norden forøvrig')[nordisk+1]
